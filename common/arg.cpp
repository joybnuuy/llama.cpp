...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--moe-pool-size"}, "N",
        "enable sentence-level MoE expert GPU pool (B2 VRAM saving). "
        "N=0 disables (default), N>0 sets max experts per layer in the pool",
        [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg(
        {"--moe-pool-full-layers"}, "N",
        "first N layers load all experts into VRAM (no pooling, 100%% hit rate). "
        "Remaining layers use --moe-pool-size. Default: 0 (all layers pooled).",
        [](common_params & params, const std::string & value) {
            params.moe_pool_full_layers = std::stoi(value);
        }
    ).set_env("LLAMA_ARG_MOE_POOL_FULL_LAYERS"));
